...
def yellow_with_rip
  ...
  style(
    ...
    waveform_base: { stroke_width: 1.8 },
...
t = Tango::Scope.new do
  yellow_with_rip
  fold_type type: :saw2, gap_color: '#eec', teeth: 12, gap_width: 15, gap_corner: 'miter'
  width 1100
  height 400
  channel :P4, initial: false, color: '#f90', subtext: '(Pin 4)', negative: true
  channel :P8, initial: true, color: '#e00', subtext: '(Pin 8)'
  repeat(2, 'Main Cycle', period: 14400) do |line|
    mark :cycle_start, hide: true
    label("START\nCYCLE ##{line+1}")
    end_measure('Inactive time')
    ...
    measure('Gap before pin 8 falls', y: 2.5) do
      sample 2.375, P8: false
    end
    measure('Pin 8 low time', y: 3.5, override: '5.68ms') do
      mark :pin8_low_start, hide: true
      step 8
      fold :bit_loop_begin
      mark_seek :pin8_low_start, 5676
      fold :bit_loop_end
      mark_seek :pin8_low_start, 5680
      sample 0, P8: true
      start_measure('Inactive time', y: 2.5, align: :right, override: '~ 8.5ms')
    end
    step 10
    fold :bit_loop_begin
    mark_seek :cycle_start, 14395 # Seek to 4us before the end of this cycle.
    fold :bit_loop_end
  end
end
